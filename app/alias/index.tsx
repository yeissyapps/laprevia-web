// Alias · Configuración — equipos (azar/manual), categorías y modo de puntuación.
// Misma estructura que Mímica; la categoría +18 va con estilo distintivo (rojo/oscuro).
...
import { CATEGORIAS_ALIAS, CATEGORIAS_DEFECTO_ALIAS, PICANTE_ID } from '@/data/alias';
import { RONDAS_DEFAULT_ALIAS } from '@/data/alias';
import { RONDAS_MAX, RONDAS_MIN, nombreEquipo, repartirEquipos, type FormacionEquipos, type ModoPuntuacion } from '@/data/mimica';
...
const DESBLOQUEO_PICANTE_ALIAS = 'alias-picante';

export default function AliasConfigScreen() {
  ...
  const picanteDesbloqueado = estaDesbloqueado(DESBLOQUEO_PICANTE_ALIAS);
  ...
  const [cats, setCats] = useState<string[]>(CATEGORIAS_DEFECTO_ALIAS);
  const [modo, setModo] = useState<ModoPuntuacion>('libre');
  const [rondas, setRondas] = useState(RONDAS_DEFAULT_ALIAS);
  const [anuncioVisible, setAnuncioVisible] = useState(false);

  // La sesión hidrata async tras el primer render: ajusta la asignación al nº real.
  useEffect(() => {
    setAsign((prev) => {
  ...
  const toggleCat = (id: string) => {
    // El +18 es premium: se desbloquea gratis viendo un vídeo (2,5 h).
  ...
  const desbloquearPicante = () => {
    desbloquearTemporal(DESBLOQUEO_PICANTE_ALIAS);
  ...
  const empezar = () => {
    ...
    router.push(`/alias/jugar?${q}` as Href);
  };
    ...
    <View style={[styles.screen, { paddingTop: insets.top + 10 }]}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 130 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.catLista}>
          {CATEGORIAS_ALIAS.map((cat) => {
            const marcado = cats.includes(cat.id);
            const picante = cat.id === PICANTE_ID;
            const acento = picante ? colors.red : colors.purple;
            return (
              <PressableScale
                key={cat.id}
                ...
                style={[
                  styles.catRow,
                  shadows.card,
                  picante && styles.catRowPicante,
                  marcado && { borderColor: acento },
                ]}>
                <View
                  style={[
                    styles.check,
                    marcado ? { backgroundColor: acento, borderColor: acento } : { borderColor: colors.border },
                  ]}>
                  {marcado && <Text style={styles.checkIcon}>✓</Text>}
                </View>
                <Text style={[styles.catNombre, picante && styles.catNombrePicante]}>
                  {picante ? '+18 🔥' : cat.nombre}
                </Text>
                {picante && (
                  <View style={[styles.picanteBadge, !picanteDesbloqueado && { backgroundColor: colors.lav100 }]}>
                    <Text style={[styles.picanteText, !picanteDesbloqueado && { color: colors.purple }]}>
                      {picanteDesbloqueado ? 'SOLO ADULTOS' : '📺 GRATIS CON VÍDEO'}
                    </Text>
                  </View>
                )}
              </PressableScale>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingHorizontal: 26, marginBottom: 8 },
  backBtn: {
    width: 40,
    height: 40,
    borderRadius: 20,
  ...
  formCard: { flex: 1, borderRadius: 18, paddingVertical: 18, alignItems: 'center', gap: 2, borderWidth: 2 },
  ...
  catRowPicante: { backgroundColor: '#FFF5F4', borderColor: '#F6C9C2' },
  ...
  catNombrePicante: { color: colors.red },
  picanteBadge: { backgroundColor: colors.ink, borderRadius: 30, paddingHorizontal: 10, paddingVertical: 4 },
  picanteText: { fontFamily: fonts.bodyX, fontSize: 10, color: colors.white, letterSpacing: 0.8 },
  segmento: { flexDirection: 'row', backgroundColor: colors.ghost, borderRadius: 14, padding: 4, gap: 4, marginTop: 12 },
  ...
  stepBtnPlus: { width: 36, height: 36, borderRadius: 18, backgroundColor: colors.purple, alignItems: 'center', justifyContent: 'center' },
  stepOff: { opacity: 0.4 },
  ...
  stepNum: { fontFamily: fonts.display, fontSize: 26, color: colors.ink, minWidth: 40, textAlign: 'center', includeFontPadding: false },
  footer: {
    position: 'absolute',
    left: 0,
    right: 0,
